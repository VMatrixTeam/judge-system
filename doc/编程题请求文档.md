...
| :---------- | :---------- | :----------------------------------------------------------- |
| type    | string      | 候选项："programming", "output", "choice", "program_blank_filling"分别表示编程题、程序输出题、选择题、完善程序题，其中评测 4.0 暂时只实现了编程题的接口 |
...
| 标准/随机测试<br>且不使用自定义比较器 | standard | standard | 如果题目允许多余空格/空行，选择 diff-ign-space；<br>如果题目要求输出完全一致，选择 diff-all； |

...
void to_json(json &j, const error_report &report) {
    j = {{"message", ensure_utf8(report.message)},
         {"result", ensure_utf8(report.result)},
         {"continue", false},
         {"grade", 0}};
}
...
void to_json(json &j, const compile_check_report &report) {
    j = {{"continue", report.cont},
         {"grade", report.grade},
         {"compile_error_log", ensure_utf8(report.error_log)},
         {"compile check", ensure_utf8(report.message)}};
}

void to_json(json &j, const check_case_report &report) {
    j = {{"stdin", ensure_utf8(report.stdin)},
         {"result", ensure_utf8(report.result)},
         {"stdout", ensure_utf8(report.stdout)},
         {"message", ensure_utf8(report.message)},
         {"timeused", report.timeused},
         {"memoryused", report.memoryused},
         {"standard_stdout", ensure_utf8(report.standard_stdout)}};
}
...
void to_json(json &j, const memory_check_error_report &report) {
    if (report.message.empty())
        j = {{"valgrindoutput", report.valgrindoutput},
             {"stdin", ensure_utf8(report.stdin)}};
    else {
        j = {{"message", ensure_utf8(report.message)},
             {"stdin", ensure_utf8(report.stdin)}};
    }
...
void to_json(json &j, const gtest_check_report &report) {
    ...
    if (report.error_message.empty()) {
        ...
        failure += {{"error", ensure_utf8(report.error_message)}};
    }

 ...
 */
std::string read_file_content(const std::filesystem::path &path, long max_bytes = -1);
 ...
 * @return 文本文件的内容(没有指定编码)
 */
std::string read_file_content(std::filesystem::path const &path, const std::string &def, long max_bytes = -1);

...
struct monitor {
     ...
     * @param message 错误原因
     */
    virtual void report_error(int worker_id, const std::string &message);
    ...
    virtual void interrupt_submissions();

    virtual void interrupt_judge_tasks();

    /**
     * @brief 监控上报完成一个编程题提交评测所用的时间
     * @param submit 提交
     */
    virtual void get_judge_time(submission &submit);
};

...
protected:
    ...
    void prepare(programming_submission &prog, const string &random, const string &standard, const string &source) {
        prog.category = "mock";
        prog.prob_id = "1234";
        prog.sub_id = "12340";
        ...
        auto standard_ptr = make_unique<source_code>();
...
#define TEST_TASK(random_source, standard_source, submission_source, compilation_stage, random_stage) \
    do {                                                                                              \
        ...
        prepare(prog, random_source, standard_source, submission_source);                             \
...
TEST_F(RandomCheckerTest, SubmissionRETest) {
    TEST_TASK(RANDOM_AC, STANDARD_AC, STANDARD_RE, status::ACCEPTED, status::SEGMENTATION_FAULT);
}

...
protected:
    ...
    void prepare_only_memory_check(programming_submission &prog, const string &source) {
        prog.category = "mock";
        prog.prob_id = "1234";
        prog.sub_id = "12340";
    ...
    void prepare_with_standard(programming_submission &prog, const string &source) {
        prog.category = "mock";
        prog.prob_id = "1234";
        prog.sub_id = "12340";
        prog.updated_at = chrono::system_clock::to_time_t(chrono::system_clock::now());
    ...
    void prepare_with_random(programming_submission &prog, const string &source) {
        prog.category = "mock";
        prog.prob_id = "1234";
        prog.sub_id = "12340";
        prog.updated_at = chrono::system_clock::to_time_t(chrono::system_clock::now());
        ...
        submission->source_files.push_back(make_unique<text_asset>("main.cpp", source));
        prog.submission = move(submission);
        auto standard = make_unique<source_code>();
        ...
        auto random = make_unique<source_code>();
        random->language = "cpp";
...
TEST_F(MemoryCheckerTest, SpecialTest) {
    TEST_TASK(STANDARD_SPECIAL, prepare_only_memory_check, status::WRONG_ANSWER, status::WRONG_ANSWER, false);
}
...
TEST_F(MemoryCheckerTest, DependsOnStandardTimeLimitExceededTest) {
    TEST_TASK(STANDARD_TLE, prepare_with_standard, status::TIME_LIMIT_EXCEEDED, status::DEPENDENCY_NOT_SATISFIED, false);
}

TEST_F(MemoryCheckerTest, DependsOnStandardWrongAnswerTest) {
    TEST_TASK(STANDARD_WA, prepare_with_standard, status::WRONG_ANSWER, status::DEPENDENCY_NOT_SATISFIED, false);
}
...
TEST_F(MemoryCheckerTest, DependsOnRandomTimeLimitExceededTest) {
    TEST_TASK(STANDARD_TLE, prepare_with_random, status::TIME_LIMIT_EXCEEDED, status::DEPENDENCY_NOT_SATISFIED, false);
}

TEST_F(MemoryCheckerTest, DependsOnRandomWrongAnswerTest) {
    TEST_TASK(STANDARD_WA, prepare_with_random, status::WRONG_ANSWER, status::DEPENDENCY_NOT_SATISFIED, false);
}

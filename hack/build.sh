...
pushd build && cmake -DRabbitmqc_DIR=$(pwd)/ext/rabbitmq-c -DBoost_USE_STATIC_LIBS=ON -DCMAKE_INSTALL_PREFIX=${INSTALL_DIR} -DCPR_BUILD_TESTS=OFF -DCPR_BUILD_TESTS_SSL=OFF -DBUILD_ENTRY=ON -DBUILD_SHARED_LIBS=OFF -G"Ninja" $@ .. && ninja && popd
